...
FUNCTIONCODEBLOCKP = gdb.lookup_type("void").pointer()
...
class CodeBlock:
    ...
    def inferredName(self):
        return wtf_string_to_utf8(self._applyNoArgMethod("inferredName"))
    ...
    def _applyNoArgMethod(self, method):
        return gdb.parse_and_eval("((CodeBlock *)0x%x)->%s()" % \
...
def codeblockname(args, from_ttytty):
    if len(args) != 1:
        print "syntax: codeblockname codeblock"
        return
    codeBlockptr = gdb.parse_and_eval("(CodeBlock*) %s" % args[0])
    codeBlock = CodeBlock(codeBlockptr)
    print codeBlock.inferredName()

gdb_command("codeblockname", codeblockname)


#gdb_break("JSC::JIT::privateCompile")
#gdb_break("JSC::MIPSAssembler::jal")
#gdb_break("JSC::MIPSAssembler::jalr")
#gdb_break("JSC::linkCallGenerator")
#gdb_break("JSC::JIT::emitNakedCall");
#gdb_break("JIT.cpp:733");
#gdb_break("JIT.cpp:658");
gdb_break("JIT.cpp:802")
#gdb_break("JSC::MIPSAssembler::linkCallInternal")
